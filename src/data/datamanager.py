...
def train_val_test_split(data_frame: pd.DataFrame, shuffle=True):
    ...
    train = pd.concat([train_false, train_true]).reset_index(drop=True)
    val = pd.concat([val_false, val_true]).reset_index(drop=True)
    test = pd.concat([test_false, test_true]).reset_index(drop=True)
...
def clean(data_frame: pd.DataFrame):
    return data_frame.drop_duplicates(subset="func", keep='first')


def drop(data_frame: pd.DataFrame, keys):
    data_frame.drop(keys, axis=1, inplace=True)
    # for key in keys:
    #     del data_frame[key]
